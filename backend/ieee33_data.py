"""
Standard IEEE 33-bus radial distribution test feeder.

Source: Baran, M.E. & Wu, F.F., "Network reconfiguration in distribution
systems for loss reduction and load balancing", IEEE Trans. Power
Delivery, 1989. This is the same per-unit base (12.66 kV, 10 MVA) and
branch/load table used by most published hosting-capacity studies on
this benchmark, including pandapower's bundled `case33bw()`.

Only the 32 in-feeder (normally-closed) branches are included here - the
5 normally-open tie switches from the original paper are omitted, since
this project doesn't model switch state.
"""

VN_KV = 12.66
BASE_MVA = 10.0

# (from_bus, to_bus, r_ohm, x_ohm) - 1-indexed bus numbers, bus 1 = substation
BRANCHES = [
...
# (bus, p_kw, q_kvar) - bus 1 (substation) carries no load
LOADS = [
